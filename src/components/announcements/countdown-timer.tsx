...
const TimeUnit = ({ label, value }: { label: string; value: string }) => (
  <div className="flex flex-col items-center gap-1 bg-white w-[35px] h-[45px] max-sm:w-[24px] max-sm:h-[30px] max-md:w-[28px] max-md:h-[36px] rounded-[6px]">
    <div className="text-text-primary max-md:text-fluid-sm font-bold pt-0.5 px-3 rounded min-w-10 text-center">
      {value}
    </div>
    <div className="text-fluid-xs font-bold text-text-primary tracking-widest">{label}</div>
  </div>
);
...
export default function SaleTimer() {
  ...
  return (
    <div className="md:p-4 max-md:pl-[6px] flex justify-center items-center gap-4 text-fluid-base">
